...
contract AtomicSwapEscrowDest {
    ...
    function destRevealSecret(string memory orderId, address owner, bytes memory secret) external {
        bytes32 key = _escrowKey(orderId, owner);
    ...
    function destCancelEscrow(string memory orderId, address owner) external {
        bytes32 key = _escrowKey(orderId, owner);
        Escrow storage esc = escrows[key];
        if (esc.status != STATUS_ACTIVE) revert EscrowNotActive();
        if (esc.owner == address(0)) revert EscrowNotFound();
    ...
    function destEscrowExists(string memory orderId, address owner) external view returns (bool) {
        bytes32 key = _escrowKey(orderId, owner);
        return escrows[key].owner != address(0);
    }

    // View: get escrow details
    function destGetEscrow(string memory orderId, address owner) external view returns (
        string memory,
        bytes32,
        address,
        address,
    ...
    function destIsEscrowActive(string memory orderId, address owner) external view returns (bool) {
        bytes32 key = _escrowKey(orderId, owner);
        return escrows[key].status == STATUS_ACTIVE;
    }

    // View: is timelock expired
    function destIsTimelockExpired(string memory orderId, address owner) external view returns (bool) {
        bytes32 key = _escrowKey(orderId, owner);
        Escrow storage esc = escrows[key];
        if (esc.owner == address(0)) revert EscrowNotFound();
