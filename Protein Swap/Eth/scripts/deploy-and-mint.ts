...
async function main() {
  ...
  try {
    ...
    console.log("\n🪙 Minting 1000 UNITE tokens to deployer...");
    const mintAmount = ethers.parseUnits("1000", 18); // 1000 tokens with 18 decimals
    const mintTx = await unite.connect(wallet).mint(wallet.address, mintAmount, { nonce: nonce++ });
    const mintReceipt = await mintTx.wait();
    const mintTxHash = mintReceipt?.hash;
    console.log(`✅ Minted 1000 UNITE tokens to: ${wallet.address}`);
    ...
    const txExplorerBase = "https://sepolia.basescan.org/tx/";
    ...
    if (mintTxHash) {
      console.log(`Mint Transaction: ${txExplorerBase}${mintTxHash}`);
    }
    ...
    console.log(`⛽ Used ${nonce - currentNonce} transactions`);
